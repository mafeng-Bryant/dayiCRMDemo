...
- (UIView *)viewForMessage:(NSString *)message title:(NSString *)title image:(UIImage *)image {
    ...
    if (title != nil) {
        ...
        if (IOS7)
        {
            ...
            NSMutableDictionary *dic = [NSMutableDictionary dictionaryWithDictionary:[attrStr attributesAtIndex:0 effectiveRange:&range] ];   // 获取该段attributedString的属性字典
            [dic setObject:titleLabel.font forKey:NSFontAttributeName];
            
            // 计算文本的大小
            expectedSizeTitle = [title boundingRectWithSize:maxSizeTitle // 用于计算文本绘制时占据的矩形块
    ...
    if (message != nil) {
        ...
        if (IOS7)
        {
            ...
            NSRange range = NSMakeRange(0, attrStr.length);
            NSMutableDictionary *dic = [NSMutableDictionary dictionaryWithDictionary:[attrStr attributesAtIndex:0 effectiveRange:&range] ];   // 获取该段attributedString的属性字典
            [dic setObject:messageLabel.font forKey:NSFontAttributeName];
            
            // 计算文本的大小
            expectedSizeMessage = [message boundingRectWithSize:maxSizeMessage // 用于计算文本绘制时占据的矩形块
                                                    options:NSStringDrawingUsesLineFragmentOrigin // 文本绘制时的附加选项
                                                 attributes:dic        // 文字的属性
                                                    context:nil].size; // context上下文。包括一些信息，例如如何调整字间距以及缩放。该对象包含的信息将用于文本绘制。该参数可为nil
        }

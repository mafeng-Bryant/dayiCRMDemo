...
@interface ReserveListViewController ()
...
- (void)viewDidLoad {
	...
	else
	{
		...
		[btn1 setImage:[UIImage imageNamed:@"add_member"] forState:UIControlStateNormal];
		[btn1 setFrame:CGRectMake(0.0f, 0.0f, 35, 28)];
		[btn1 addTarget:self action:@selector(btn_list_click:) forControlEvents:UIControlEventTouchUpInside];
		btn_list = [[UIBarButtonItem alloc] initWithCustomView:btn1];
    ...
    [[NSNotificationCenter defaultCenter]addObserver:self selector:@selector(loadData) name:@"reserveDidAdd" object:nil];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
	...
	ctrl.reserveId = [[buffer objectAtIndex:indexPath.section] objectForKey:@"id"];
	ctrl.srcType = 1;
...
{
	RoomListViewController *ctrl = [[RoomListViewController alloc]initWithStyle:UITableViewStyleGrouped];
	[self.navigationController pushViewController:ctrl animated:YES];
	if ([self.navigationController respondsToSelector:@selector(interactivePopGestureRecognizer)]){
		self.navigationController.interactivePopGestureRecognizer.delegate = nil;
	}
...
- (void)loadData
{
	...
	NSString *full_url = [NSString stringWithFormat:@"%@%@", BASE_URL,RESERVE_LIST_URL];
...
- (void) onLoaded: (NSNotification *)notify
{
	...
	NSArray *arr = [data objectForKey:@"reserveList"];
	if ([arr count] == 0) {
		[buffer removeAllObjects];
		[self.tableView reloadData];
		return;
